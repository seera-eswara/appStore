...
class AppStore extends Component {
  ...
  activeTab = id => this.setState({selectedTab: id})
  ...
  render() {
    ...
    const filteredAppList = appsList.filter(app => selectedTab === app.category)
    const finalList = filteredAppList.filter(app =>
      app.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )

    return (
      <div className="appStoreCont">
        <h1>App Store</h1>
        ...
        <ul className="tabs-list">
          {tabsList.map(tab => (
            <TabITem
              key={tab.tabId}
              tabId={tab.tabId}
              tabText={tab.displayText}
              activeTab={this.activeTab}
              isActive={selectedTab === tab.tabId}
            />
          ))}
        </ul>
